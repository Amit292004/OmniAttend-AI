...
export default function AttendancePage() {
  ...
  const fetchData = async (currentUser: any) => {
    try {
      ...
      if (isTeach) {
        // Teacher
        const res = await fetch(`http://localhost:8000/api/attendance/teacher/${currentUser.teacher_id}`);
        if (res.ok) {
        ...
        // Student
        const res = await fetch(`http://localhost:8000/api/attendance/student/${currentUser.student_id}`);
        if (res.ok) {
          const data = await res.json();
  ...
  const handleDeleteSession = async (subjectId: number, timestamp: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:8000/api/attendance/session?subject_id=${subjectId}&timestamp=${timestamp}`, {
        method: "DELETE"
      });
      if (res.ok) {
  ...
  const handleExportReport = () => {
    ...
    const url = `http://localhost:8000/api/attendance/export?teacher_id=${user.teacher_id}&subject_code=${exportSubject}&start_date=${exportStartDate}&end_date=${exportEndDate}`;
