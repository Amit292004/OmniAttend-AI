...
export default function SubjectsPage() {
  ...
  const fetchSubjects = async (currentUser: any) => {
    try {
      ...
      const url = isTeach
        ? `http://localhost:8000/api/subjects/teacher/${currentUser.teacher_id}`
        : `http://localhost:8000/api/subjects/student/${currentUser.student_id}`;
  ...
  const handleEnrollSubject = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const codeRes = await fetch(`http://localhost:8000/api/subjects/code/${enrollCode}`);
      if (!codeRes.ok) {
      ...
      // 2. Enroll
      const enrollRes = await fetch("http://localhost:8000/api/subjects/enroll", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
